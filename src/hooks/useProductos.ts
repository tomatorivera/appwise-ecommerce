...
import { Producto, ProductoResponse } from '../types/producto.types'
...
export function useProduct(id: number) {
  const [status, setStatus] = useState<EstadoAsync<Producto>>({
    status: 'idle',
  })
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error(
            `Ocurrió un error obteniendo el producto de ID ${id} - HTTP STATUS: ${response.status}`
          )

        const responseData: Producto = await response.json()
